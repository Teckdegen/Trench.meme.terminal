...
import { UserAvatar, HandleLink } from "@/components/Handle";
import { ModalShell } from "@/components/ui/modal-shell";

export const Route = createFileRoute("/feed")({ component: Social });
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center justify-end mt-2">
          <div className="flex items-center gap-2">
            {/* X-style char counter */}
            <span
              className={`text-[11px] tabular-nums ${
                body.length > 180 ? (body.length >= 200 ? "text-down font-bold" : "text-yellow-500") : "text-muted-foreground"
              }`}
            >
              {200 - body.length}
            </span>
            <button
              onClick={submit}
              disabled={!body.trim() || body.length > 200 || posting || (supabaseLive && !me)}
